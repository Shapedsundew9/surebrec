...
@pytest.mark.parametrize("s_seed, v_seed", [(1567657329, 1685125724), (1685125724, 1685125724)])
def test_lgc_json_dump_entry_validator_nasty_seeds(s_seed, v_seed) -> None:
    ...
    generate(LGC_json_dump_entry_validator, 1, s_seed=s_seed, v_seed=v_seed, validate=True)
...
def test_random_seed_2() -> None:
    ...
    v_seed = 8
    a: list[dict[Any, Any]] = generate(raw_table_config_validator, 10, s_seed, v_seed)
    b: list[dict[Any, Any]] = generate(raw_table_config_validator, 10, s_seed, v_seed + 3)
    if a[3:] != b[:-3]:
        for num, (_a, _b) in enumerate(zip(a, b)):
            _logger.debug(
                f"Element {num} does not match:\n"
